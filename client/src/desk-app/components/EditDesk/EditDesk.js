...
import { editDesk } from "../../../redux/actions/desk";
...
function EditDesk() {
  ...
  const [selected, setSelected] = useState({
    "Desk Base": [`${BASE_URL}/desk-assets/deskBase_0.png`],
    "Top Shelf": [],
    "Bottom Shelf": [],
    Door: [],
  });
  ...
  const [isButtonSaveDeskDisabled, setIsButtonSaveDeskDisabled] =
    useState(false);
  ...
  const [preview, setPreview] = useState(
    `${BASE_URL}/desk-assets/defaultClaimedDesk.png`
  );
  const [imageInfo, setImageInfo] = useState({});
  const isDeskAlreadyTaken = useSelector(
    (state) => state?.session?.isDeskAlreadyTaken
  );
  ...
  const handleOpenModalWithVariations = (item, type) => {
    ...
    const currentSelectionVariation = currentSelection
      ? currentSelection.replace(`${BASE_URL}/desk-assets/`, "")
      : null;
        ...
        (acc, category) => {
          ...
          acc[category] = [
            urlParams.get(categoryKey1) &&
              `${BASE_URL}/desk-assets/${urlParams.get(categoryKey1)}.png`,
            urlParams.get(categoryKey2) &&
              `${BASE_URL}/desk-assets/${urlParams.get(categoryKey2)}.png`,
          ].filter(Boolean);
          return acc;
        },
        {}
  ...
  const updateDesk = (type, image, item) => {
      ...
      updatedSelection[type] = updatedSelection[type].filter((selectedItem) => {
        if (item && item.hasVariation) {
          return !item.variations.some(
            (variation) =>
              `${BASE_URL}/desk-assets/${variation}` === selectedItem
          );
        }
        return item && selectedItem !== `${BASE_URL}/desk-assets/${item.name}`;
      });
    } else {
    ...
    if (
      ...
      !updatedSelection[type].some((selectedItem) =>
        requiredItem.variations.includes(
          selectedItem.replace(`${BASE_URL}/desk-assets/`, "")
        )
      )
    ) {
      ...
      updatedSelection[type].unshift(
        `${BASE_URL}/desk-assets/${requiredItem.variations[0]}`
      );
    }

    // Garante que o item isRequired do "Desk Base" fique sempre no início do array
    if (type === "Desk Base") {
  ...
  const handleSaveToBackend = async () => {
    try {
      setIsButtonSaveDeskDisabled(true);
      await dispatch(editDesk(imageInfo));
    } catch (error) {
      console.error("Error editing desk:", error);
    } finally {
      setIsButtonSaveDeskDisabled(false);
    }
  };
  ...
  if (isDeskAlreadyTaken) {
    return (
      <>
        <div className={`wrapper ${visitor?.isAdmin ? "mt-90" : ""}`}>
          <h1>This desk is already taken</h1>
          <p>Please select another desk!</p>
        </div>
      </>
    );
  }
    ...
    <>
        ...
        <ItemVariationSelectorModal
          ...
          onSelect={(selectedVariation) => {
            if (selectedVariation) {
              const imageUrl = `${BASE_URL}/desk-assets/${selectedVariation}`;
              updateDesk(currentItem.type, imageUrl, currentItem);
            } else {
              updateDesk(currentItem.type, null, currentItem);
            }
          }}
      ...
      <div className={`wrapper ${visitor?.isAdmin ? "mt-90" : ""}`}>
              ...
              <div style={{ marginBottom: "10px", textAlign: "left" }}>
                {categories[type].map((item) => (
                  ...
                  >
                    ...
                    <img
                      src={`${BASE_URL}/desk-assets/${item.name}`}
                      alt={item.name}
                      ...
                      onClick={() => {
                        if (item.hasVariation) {
                          ...
                          updateDesk(
                            type,
                            `${BASE_URL}/desk-assets/${item.name}`,
                            item
                          );
                        }
                      }}
        ...
        {Object.keys(validationErrors).length > 0 && (
          <p style={{ color: "red" }}>
            Please select an item from each category to build the desk.
          </p>
        )}
        ...
        <div className="footer-wrapper">
          <div className="footer-fixed" style={{ backgroundColor: "white" }}>
            <button
              ...
              disabled={
                !selected["Desk Base"].length > 0 || isButtonSaveDeskDisabled
              }
            >
              Save
            </button>
