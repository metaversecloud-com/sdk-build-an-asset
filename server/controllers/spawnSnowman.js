...
export const spawnSnowman = async (req, res) => {
  try {
    ...
    if (host === "localhost") {
      BASE_URL = `https://basset0-dev-topia.topia-rtsdk.com`;
    } else {
    ...
    await removeAllUserAssets(urlSlug, profileId, credentials);
...
function calculateCompleteImageName(imageInfo) {
...
async function removeAllUserAssets(urlSlug, profileId, credentials) {
  const world = await World.create(urlSlug, { credentials });

  try {
    const spawnedAssets = await world.fetchDroppedAssetsWithUniqueName({
      uniqueName: `snowmanSystem-${profileId}`,
    });
...
async function dropImageAsset({
  ...
  const spawnedAssetUniqueName = `snowmanSystem-${visitor?.profileId}`;
